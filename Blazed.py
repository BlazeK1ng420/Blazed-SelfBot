...
try: 
    ...
    def update_config():
        ...
        if ("poke_detect" in configFile):
            configJson.pop("poke_detect")
            print(printSpaces+"Removing poke detect from config.")

        if ("poke" not in configJson["webhooks"]):
            configJson["webhooks"]["poke"] = ""
            print(printSpaces+"Adding poke webhook to config.")
        ...
        if ("detections" not in configFile):
            ...
            configJson["detections"]["poke"] = bool(True)
            print(printSpaces+"Adding poke detection to config.")
        ...
        if ("ignored_servers" not in configFile):
            ...
            configJson["ignored_servers"]["pokes"] = []
            print(printSpaces+"Adding pokes ignored servers to config.")
...
async def example(Blaze):
    ...
    **\_\_deletetimeout\_\_** : Config delete timeout (seconds).
    ...
    __pokewebhook__ = CONFIG["webhooks"]["poke"]
    ...
    __pokedetect__ = CONFIG["detections"]["poke"]
    ...
    async def on_message_delete(message):
        if __blazeloaded__:
            ...
            if Config.getConfig()["detections"]["poke"]:
                if Blaze.user.mentioned_in(message):
                    if message.guild.id not in __ignoredservers__["pokes"]:
                        ...
                        if __pokewebhook__ != "":
                            webhook = DiscordWebhook(url=__pokewebhook__)
...
`{Blaze.command_prefix}`**masspoke [delay] [@user]** » Blaze Ping the user in every channel.
`{Blaze.command_prefix}`**poke [@user]** » Ping a user then delete the message.
...
```""")          
    ...
    @Blaze.command(name="poke", description="Ping a user then delete the message.", usage="poke [@user]")
...
```""", delete_after=__deletetimeout__)
    ...
    async def webhooksetup(ctx):
        global __nitrowebhook__, __privnotewebhook__, __giveawaywebhook__, __pokewebhook__, __friendsupdatewebhook__, __dmtypingwebhook__, __guildleavewebhook__, __selfbotwebhook__, __ticketswebhook__
        ...
        __pokewebhook__ = blazePingWebhook.url
        ...
        configFile["webhooks"]["poke"] = __pokewebhook__
...
# {__embedfooter__}```""", delete_after=__deletetimeout__)              
    ...
    @Blaze.command(name="masspoke", description="Ping a mass amount of people in the command server and delete the messages.", usage="masspoke (amount of messages) (send delay)", aliases=["massblazemention", "theotherfunny"])
    async def masspoke(ctx, amount:int=1, delay:int=0):
        if __riskmode__:
            try:
                await ctx.message.delete()
            except:
